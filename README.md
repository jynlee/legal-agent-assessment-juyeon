...
**원천 자료를 직접 살펴보셔도 괜찮습니다.** 그래서 출처를 추가해 놓았습니다.
...
있다면 — **gyro에게 말씀 주십시오. 커버리지는 변경 가능합니다.**

레코드마다 원천이 요구하는 발행 표기가 실려 있습니다. **답변에 포함해 주십시오.** 판례를
...
**작업 시작 전 릴리스 검증을 부탁드립니다.** 전달된 압축 파일의 `DELIVERY.md` 안내에
따라 압축 파일의 SHA-256을 확인합니다. 압축을 푼 뒤 `scripts/verify_release.py`가 모든 레코드 콘텐츠
해시를 재계산하고, 레코드 수와 커버리지를 매니페스트와 대조하며, 오류가 있으면 0이
아닌 코드로 종료합니다. 이 검사로 어떤 동결 릴리스를 전달받았는지와 그 레코드가
MZO 재실행에 여전히 유효한지를 확인합니다.
...
파싱·AWS 서비스 접근 권한은 별도 채널로 전달됩니다.
