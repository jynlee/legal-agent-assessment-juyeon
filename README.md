...
않는 판례는 수집하지 않았습니다. 공식 가이드는 보건복지부와 식품의약품안전처가
발간한 정부 간행물입니다.
...
판례에는 레코드마다 원문 URL이 들어 있어 어떤 판례든 원본과 대조할 수 있고, 가이드는
공식 발간 번호를 갖습니다. API는 공개돼 있으나 `OC` id는 **개발자별로 발급**되므로,
직접 질의할 때는 본인이 발급받은 id를 쓰십시오.
...
릴리스를 만든 스크립트가 [scripts/](scripts/)에 있습니다. 가이드는 PDF를 문서 파싱
API로 처리해 만들고, 판례는 수집된 JSONL을 매핑해 만들며, 매니페스트는 커버리지를
**타이핑이 아니라 레코드에서 세어** 기록합니다. 어떤 레코드가 왜 그렇게 생겼는지
알아보는 가장 빠른 방법은 이 코드를 읽는 것입니다.
...
이 두 값은 **MZO가 원본을 받은 시각과 가이드를 파싱한 시각**입니다. 여러분 머신에 관한
사실이 아니라 **릴리스의 입력**이며, 레코드가 이 값을 담고 있기 때문에 반드시 넘겨야
합니다. 생략하면 빌드마다 자기 시계와 파일 시각을 찍어, **만들어진 시각만 다른** 레코드가
나옵니다. 위 값을 주면 파일 해시가 매니페스트가 선언한 값과 일치합니다.

다만 **가이드 본문은 호스팅된 파서의 산출물**이고, 그 엔진과 모델은 예고 없이 바뀔 수
있습니다. `provenance.extraction`이 어떤 파서가 그 텍스트를 만들었는지 기록하는 이유가
이것입니다. 나중에 다시 돌려 결과가 다르면 **파서가 움직인 것이지 릴리스가 틀린 것이
아닙니다.** 재빌드 결과가 매니페스트와 어긋나면 자기 산출물을 채택하지 말고 보고하십시오.
