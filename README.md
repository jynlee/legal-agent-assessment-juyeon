...
같은 명시적 타임스탬프에서 결정론적입니다 — `--acquired-at`, `--parsed-at`을 주면
바이트 단위로 같은 결과가 나옵니다. 다만 **가이드 본문은 호스팅된 파서의 산출물**이고,
그 엔진과 모델은 예고 없이 바뀔 수 있습니다. `provenance.extraction`이 어떤 파서가
그 텍스트를 만들었는지 기록하는 이유가 이것입니다. 나중에 다시 돌려 결과가 다르면
**파서가 움직인 것이지 릴리스가 틀린 것이 아닙니다.** 재빌드 결과가 매니페스트와
어긋나면 자기 산출물을 채택하지 말고 보고하십시오.
